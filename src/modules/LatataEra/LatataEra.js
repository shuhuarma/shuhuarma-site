...
class LatataEra extends Component {
	...
	loadData = () => {
		const latata1thek = '9mQk7Evt6Vs';
		const latataOfficial = 'p4RIhcY7V3c';
		axios.get(`https://www.googleapis.com/youtube/v3/videos?key=AIzaSyAmCEDPnXeQf4c0MHkuTXAMwXolZDCT1J0&id=${latata1thek}&part=statistics,snippet`)
			.then(response => {
		...
		axios.get(`https://www.googleapis.com/youtube/v3/videos?key=AIzaSyAmCEDPnXeQf4c0MHkuTXAMwXolZDCT1J0&id=${latataOfficial}&part=statistics,snippet`)
			.then(response => {
					if (response.status === 200) {
						this.setState({
							isLoaded2: true,
