...
import * as styles from './SenoritaEra.module.scss';
import MusicVideoItem from '../YoutubeData/components/MusicVideoItem/MusicVideoItem';

class SenoritaEra extends Component {
	constructor(props) {
		super(props);
		this.state = {
			isLoaded1: false,
	...
	loadData = () => {
		const senorita1thek = 'G8APgeFfkAk';
		const senoritaOfficial = '2cevbhEqQF4';
		axios.get(`https://www.googleapis.com/youtube/v3/videos?key=AIzaSyAmCEDPnXeQf4c0MHkuTXAMwXolZDCT1J0&id=${senorita1thek}&part=statistics,snippet`)
			.then(response => {
		...
		axios.get(`https://www.googleapis.com/youtube/v3/videos?key=AIzaSyAmCEDPnXeQf4c0MHkuTXAMwXolZDCT1J0&id=${senoritaOfficial}&part=statistics,snippet`)
			.then(response => {
					if (response.status === 200) {
						this.setState({
							isLoaded2: true,
